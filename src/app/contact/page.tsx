    ...
    <div className="min-h-screen bg-[#06080F] text-slate-200 font-sans antialiased pb-20">
      <header className="border-b border-slate-800 bg-[#06080F]/80 backdrop-blur sticky top-0 z-40">
        <div className="max-w-4xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <span className="text-xs font-bold text-emerald-400">Institutional Desk</span>
        </div>
      </header>

      <main className="max-w-4xl mx-auto px-4 sm:px-6 pt-12">
        <h1 className="text-3xl sm:text-4xl font-black text-white mb-3">Investor &amp; Wholesaler Desk</h1>
        <p className="text-xs sm:text-sm text-slate-400 mb-8">
          Submit off-market multi-family contracts or contact our underwriting team.
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="space-y-4">
            <div className="bg-slate-900/40 border border-slate-800 p-5 rounded-2xl">
              ...
              <div className="text-xs font-bold text-slate-400 uppercase">Support &amp; Inquiries</div>
            ...
            <div className="bg-slate-900/40 border border-slate-800 p-5 rounded-2xl">
              ...
              <div className="text-xs font-bold text-slate-400 uppercase">Deal Pipeline Desk</div>
              <div className="text-xs text-slate-300 mt-1">Direct off-market contract assignment ingestion across high-cap US metro markets.</div>
            </div>
          </div>
          ...
          <div className="md:col-span-2 bg-slate-900/40 border border-slate-800 p-6 sm:p-8 rounded-3xl">
            {submitted ? (
              <div className="text-center py-10 space-y-3">
                <div className="text-emerald-400 font-bold text-lg">Message Dispatched!</div>
                <p className="text-xs text-slate-400">Our desk will review your inquiry and follow up shortly.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase mb-1">Inquiry / Property Submission</label>
                  <textarea
                    rows={5}
                    required
                    value={message}
                    onChange={(e) => setMessage(e.target.value)}
                    placeholder="Provide details regarding your subscription, inquiry, or off-market contract..."
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-xs text-white placeholder-slate-600 focus:outline-none focus:border-emerald-400"
                  ></textarea>
                </div>
